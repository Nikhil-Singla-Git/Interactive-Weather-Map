...
router.post("/passwordChangeRequest", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updateQuery =
      "ALTER TABLE users ADD COLUMN IF NOT EXISTS otp INTEGER";
...
const sendMailSign = async (email: string, name: string) => {
  const transporter = nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 587, // You can also use port 465 for SSL
    secure: false, // true for port 465, false for other ports
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Welcome to WeatherMap</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            line-height: 1.6;
...
<body>
    <div class="container">
        ...
        <p>We're excited to have you with us. With WeatherMap</p>
      
        <p>Explore and stay informed with the latest weather conditions. If you need any help, our support team is here for you.</p>
        <p>Welcome aboard!</p>
        <p>Best,<br>The WeatherMap Team</p>
    </div>
</body>
</html>
`;
